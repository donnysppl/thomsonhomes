...
export default function Editnewcat() {
  ...
  useEffect(() => {
    ...
    const oldCateData = async () => {
      await fetch(nodeurl + `product/category/list/${id}`, {
        method: 'GET',
      }).then(res => res.json())
        .then(res => {
          ...
          if (res.status === 200) {
            ...
            setcatagimg(res.response.cateimg);
            setpreviewImg(nodeurl + res.response.cateimg);
          }
          else {
            alert(res.message);
          }
  ...
  const addCategoryHandle = async (e) => {
    ...
    await fetch(nodeurl + `product/category/edit/${id}`, {
      method: 'PUT',
      body: formdata,
    }).then(res => res.json())
      .then(res => {
          ...
          <div className="row">
            <div className="col-lg-10 mx-auto">
              <div className="action-header-part dashboard-bg-light rounded-3 p-4">
                <ActionHeader actiontext={'Edit Category'} actionlinktext={'Back'} actionlink={'/admin/category'} />
                <div className="form-part">
                  ...
                  <form onSubmit={addCategoryHandle} encType="multipart/form-data" >
                    ...
                    <div className="mb-3">
                      ...
                      <input type="text" className="form-control" id="slug" name='slug' onChange={inputHandle} value={inpVal.slug} />
                    </div>

                    <div className="mb-3">
                      <label htmlFor="metatitle" className="form-label">meta title</label>
                      <input type="text" className="form-control" id="metatitle" name='metatitle' onChange={inputHandle} value={inpVal.metatitle} />
                    </div>
                    <div className="mb-3">
                      <label htmlFor="metadescription" className="form-label">meta description</label>
                      <input type="text" className="form-control" id="metadescription" name='metadescription' onChange={inputHandle} value={inpVal.metadescription} />
                    </div>
                    <div className="mb-3">
                      <label htmlFor="metakeywords" className="form-label">meta keywords</label>
                      <input type="text" className="form-control" id="metakeywords" name='metakeywords' onChange={inputHandle} value={inpVal.metakeywords} />
                    </div>
                    ...
                    <div className="mb-3">
                      <div className="form-check">
                        <input className="form-check-input" type="checkbox" name='parentcate' id="parentcate"
                          onChange={checkboxHandle} value={inpVal.parentcate} />
                    ...
                    </div>
                    {
                      checkboxVal ?
                        <div className="mb-3">
                          ...
                          <select className="form-select" name='childcate' value={inpVal.childcate} defaultValue={'DEFAULT'} onChange={inputHandle}>
